...
a key issue \citep[e.g., ][]{lucy1977anumerical}. Even disk galaxies
simulated under the standard hierarchical cosmological model exhibit
instabilities \textbf{{[}CITATION{]}}. One prominent example of this
is the so-called 'angular-momentum catastrophe' \citep{navarro1994accretion}
...
in disks that are smaller than observations by a similar factor. Further,
the gas in the simulations cools too quickly causing it to spiral
...
investigated the properties of $\sim100$ local galaxies containing
an AGN with compact emission in both the X-ray and radio and showed
that the radio luminosity is well correlated with both the mass and
the X-ray luminosity. In that work, M03 defined a fundamental plane
combining the two observational signatures (X-ray and radio luminosities)
with the accretion flow onto the black hole. The existence of the
fundamental plane suggests that the physical processes regulating
the conversion of an accretion flow of gas into radiative energy could
be universal across the entire black hole mass scale.

Clearly, then, an important aspect of modern galaxy formation simulations
...
the mass and accretion rates in Illustris-2 and compare against those
measured in M03. From this analysis, we can begin to understand how
well state-of-the-art simulations replicate the real physics of black
holes in the universe to better understand the relationship between
...
rate BHs, we impose a hard lower limit \textbf{(what is the specific
...
any meaning of colors, symbols, etc. (see Fig. 3 for an example).}}
\end{figure}
\begin{figure}
...
\label{sec:analysis}\textbf{There is lots of great stuff in this
section, but it doesn't flow very well. Building a narrative is the
hardest part of writing a paper! }With the low accretion rate black
holes removed from the sample, we construct a power-law relationship
between $M$ and $\dot{M}$ using linear least-squares (Figure \ref{fig:bhpop_hist2d}).
...
\begin{figure}
\centering{}\includegraphics[clip,scale=0.6]{Figures/elvis_template}
